...
st.title("Research Paper Summarizer and Podcast Generator")
...
if st.button("Run Research"):
    ...
    if result.get("fetched", {}).get("mode") == "pdf":
        ...
        if audio_result.get("status") == "success":
            ...
            with open(audio_path, "rb") as f:
                ...
                href = f'<a href="data:audio/mp3;base64,{b64}" download="{filename}">Download Audio ({filename})</a>'
    ...
    # --- MODE: SEARCH ---
        ...
        st.subheader("Individual Paper Summaries and Audio")
        ...
        if synthesis.get("synthesis_text"):
            ...
            st.subheader(f"Cross-Paper Synthesis on: {topic}")
        ...
        if synthesis_audio.get("status") == "success":
            ...
            with open(audio_path, "rb") as f:
                ...
                b64 = base64.b64encode(audio_bytes).decode()
                filename = os.path.basename(audio_path)
                href = f'<a href="data:audio/mp3;base64,{b64}" download="{filename}">Download Podcast ({filename})</a>'
                st.markdown(href, unsafe_allow_html=True)
